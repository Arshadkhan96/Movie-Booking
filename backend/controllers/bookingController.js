...
//this function give you total amount and Also calculate based on recliner and standard seat
...
export async function getBooking(req,res){  
    try {
        if(!req.user) return res.status(401).json({success:false,message:"Authentication required to get booking"});
...
export async function deleteBooking(req,res){
    ...
    const b = await Booking.findByIdAndDelete(id).exec();
...
export async function confirmPayment(req, res) {
    try {
        ...
        if(!sessionObj) return res.status(404).json({success: true, message:"Faild to find session"});
