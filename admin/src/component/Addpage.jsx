...
const Addpage = () => {
  ...
  //To calculate total duration in minutes
  ...
  // this function create the movie
...
function Uploader({ title, onFiles, items, remove, updateMeta, icon }) {
  return (
    <div className={addMoviePageStyles.uploaderContainer}>
      <div className="flex items-center justify-between mb-3">
        ...
        <label className="text-xs px-3 py-1 bg-red-700 rounded-full cursor-pointer hover:bg-red-600 transition-colors">
          + Add
          <input 
            type="file" 
            multiple 
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
        {items && items.length > 0 ? (
          items.map((item, index) => (
            <div key={index} className="relative group bg-black/30 p-2 rounded-lg border border-red-800">
              <img 
                src={item.preview} 
                alt="Preview" 
              ...
              <button
                ...
                className="absolute -top-2 -right-2 bg-red-700 rounded-full p-1 hover:bg-red-600 transition-colors"
              >
                <X size={16} />
              </button>
              ...
              {item.name !== undefined && (
                <input
                  type="text"
                  value={item.name || ''}
                  onChange={(e) => updateMeta(index, 'name', e.target.value)}
                  placeholder="Name"
                  className="w-full p-1 text-sm bg-black/30 border border-gray-700 rounded text-white mb-1"
                />
              )}
              ...
              {item.role !== undefined && (
                <input
                  type="text"
                  value={item.role || ''}
                  onChange={(e) => updateMeta(index, 'role', e.target.value)}
                  placeholder="Role"
                  className="w-full p-1 text-sm bg-black/30 border border-gray-700 rounded text-white"
                />
              )}
            </div>
          ))
  ...
  //for latesTrailers we NamedUploader
            ...
            <div key={idx} className="flex items-center gap-3 p-2 bg-black/20 rounded-lg">
              ...
              <div className="flex-1">
                <input 
                  ...
                  placeholder="Name"
                  className="w-full p-2 text-sm bg-black/30 border border-gray-700 rounded text-white mb-1"
                />
                <div className="text-xs text-gray-400 truncate">
                  File: {item.file?.name || 'No file selected'}
                </div>
              </div>
              <button 
                type="button" 
